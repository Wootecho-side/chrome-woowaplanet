import type { Satellite } from "../components/Inventory/InventoryTypes";

export const SatelliteData: Satellite[] = [
  { id: 0, name: "special", rank: "special", icon: "🌟" },
  { id: 1, name: "star", rank: "normal", icon: "💫" },
  { id: 2, name: "flamingo", rank: "rare", icon: "🦩" },
  { id: 3, name: "earth", rank: "normal", icon: "🌍" },
  { id: 4, name: "tomato", rank: "rare", icon: "🍅" },
  { id: 5, name: "mechanic", rank: "normal", icon: "🛰️" },
  { id: 6, name: "mouse", rank: "normal", icon: "🐭" },
  { id: 7, name: "pudding", rank: "normal", icon: "🍮" },
  { id: 8, name: "cat", rank: "normal", icon: "🐱" },
  { id: 9, name: "hmm", rank: "normal", icon: "🤔" },
  { id: 10, name: "bread", rank: "normal", icon: "🍞" },
  { id: 11, name: "clover", rank: "normal", icon: "🍀" },
  { id: 12, name: "balloon", rank: "normal", icon: "🎈" },
  { id: 13, name: "ghost", rank: "normal", icon: "👻" },
  { id: 14, name: "watermelon", rank: "normal", icon: "🍉" },
  { id: 15, name: "teddy", rank: "normal", icon: "🧸" },
  { id: 16, name: "chicken", rank: "normal", icon: "🍗" },
  { id: 17, name: "onigiri", rank: "normal", icon: "🍙" },
  { id: 18, name: "beer", rank: "normal", icon: "🍺" },
  { id: 19, name: "ice", rank: "normal", icon: "🧊" },
  { id: 20, name: "baseball", rank: "normal", icon: "⚾" },
  { id: 21, name: "panda", rank: "normal", icon: "🐼" },
  { id: 22, name: "cheese", rank: "normal", icon: "🧀" },
  { id: 23, name: "pumpkin", rank: "normal", icon: "🎃" },
  { id: 24, name: "fire", rank: "normal", icon: "🔥" },
  { id: 25, name: "cloud", rank: "normal", icon: "☁️" },
];
